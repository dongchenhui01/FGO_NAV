...
private:
    ...
    void checkInitialization() {
        // 进一步降低初始化阈值，确保快速启动
        if (initialization_imu_data_.size() >= 3 && initialization_dvl_data_.size() >= 1) {
            RCLCPP_INFO(this->get_logger(), "收集到足够数据进行初始化: IMU=%zu, DVL=%zu",
                       initialization_imu_data_.size(), initialization_dvl_data_.size());
            performInitialization();
        } else {
            RCLCPP_INFO(this->get_logger(), "等待更多数据: IMU=%zu/3, DVL=%zu/1",
                        initialization_imu_data_.size(), initialization_dvl_data_.size());
        }
    }
    ...
    void optimizationCallback() {
        if (!initialized_) {
            RCLCPP_DEBUG(this->get_logger(), "优化回调: 系统未初始化");
            return;
        }

        RCLCPP_DEBUG(this->get_logger(), "执行优化回调...");

        // 执行优化
        if (fgo_->optimize()) {
            RCLCPP_INFO(this->get_logger(), "优化成功，发布结果...");
            ...
            RCLCPP_WARN(this->get_logger(), "优化失败");
        }
    }
...
private:
    ...
    // 杆臂误差补偿器
    std::unique_ptr<factor_graph_optimizer::LeverArmCompensation> lever_arm_compensator_;
